...
vim.api.nvim_create_autocmd('FileType', {
  pattern = 'markdown',
  once = true,
  callback = function() vim.pack.add { vp.gh 'iamcco/markdown-preview.nvim' } end,
})

vim.api.nvim_create_autocmd('FileType', {
  pattern = 'markdown',
  callback = function(args) vim.keymap.set('n', '<leader>cp', '<cmd>MarkdownPreview<cr>', { buffer = args.buf, desc = 'Markdown Preview' }) end,
})
